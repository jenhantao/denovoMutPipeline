...
import sys
directories = [ f for f in listdir(sys.argv[1]) if not isfile(join(sys.argv[1],f))]
allDenovo = []
filepath = sys.argv[1]
for directory in directories:
	sampleName = directory
	path = filepath+sampleName+"/"
	# read in files
	allMutationsHash = {}
	allMutations = []
	#nb_nt_file = open(sys.argv[1])
	nb_nt_file = open(path + sampleName + "_NB_NT.forSnpEff.snvs.vcf")
	...
	nb_tp_file = open(path + sampleName + "_NB_TP.forSnpEff.snvs.vcf")
	...
	nt_tp_file = open(path + sampleName + "_NT_TP.forSnpEff.snvs.vcf")
		...
		if nb_nt:
			if nb_tp:
				if nt_tp:
					...
					# print position
					allDenovo.append((mutation,allMutationsHash[mutation]["QUAL"], allMutationsHash[mutation]["FILTER"]))
...
denovoFrequencyHash = {}
for denovo in allDenovo:
	if denovo[0] in denovoFrequencyHash:
		denovoFrequencyHash[denovo[0]] += 1
	else:
		denovoFrequencyHash[denovo[0]] =1

for denovo in allDenovo:
	print denovo[0],denovo[1], denovo[2]
for denovo in denovoFrequencyHash:
	print denovo + ","+str(denovoFrequencyHash[denovo])
